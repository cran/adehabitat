...
###    Z <- sweep(Z,2,et,"/")
    ...
    corav <- crossprod(as.matrix(apply(centra1, 2, function(x) x*row.w/sum(row.w))), as.matrix(centra2))
...
hist.madifa <- function (x, scores = TRUE, type = c("h", "l"), adjust = 1, Acol,
    ...
    if (clas[j] == "f") {
      ...
      if (type == "l") {
        ...
        lines(rep(mean(tmpU), 2), c(0, densU$y[512 -
                                               sum(densU$x > mean(tmpU))]), col = Ucol, lty = 2,
              lwd = Ulwd)
        lines(densA, col = Acold, lwd = Alwd)
        lines(rep(mean(tmpA), 2), c(0, densA$y[512 -
                                               sum(densA$x > mean(tmpA))]), col = Acold, lty = 2,
              lwd = Alwd)
      }
    }
    box()
...
predict.madifa <- function (object, index, attr, nf, ...)
  {
    ...
    if ((missing(nf)) || (nf > object$nf))
      nf <- object$nf
    ll <- apply(data.frame(object$l1[, 1:nf]),1, function(x) sum(object$cw * (x^2)))
...
plot.madifa <- function(x, index, attr, xax=1, yax=2, cont=TRUE,...)
{
  ...
  foo <- function()
    {
      ...
      scatterutil.base(dfxy = x$l1[, c(xax, yax)], xax = 1, yax = 2,
                       xlim = xlim, ylim = ylim, grid = TRUE, addaxes = FALSE,
                       cgrid = 2, include.origin = TRUE, origin = c(0, 0), sub = "",
                       csub = 1.25, possub = "bottomleft", pixmap = NULL, contour = NULL,
                       area = NULL, add.plot = FALSE)
      Acol <- gray(0.8)
      Ucol <- "black"
  ...
  ka <- df2kasc(data.frame(Maha=x$mahasu,
                           mod=apply(x$l1[,c(xax,yax)],1,function(x) sqrt(sum(x^2))),
  ...
  s.arrow(x$corav, xax=xax,yax=yax,sub="Cor(habitat var., scores) available",
